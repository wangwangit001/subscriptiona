      ...
      <div class="overflow-x-auto">
        <table class="min-w-full divide-y divide-gray-200">
          <thead class="bg-gray-50">
            <tr>
              <th scope="col" class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider min-w-0 w-1/6">
                名称
              </th>
              <th scope="col" class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider min-w-0 w-1/6">
                类型
              </th>
              <th scope="col" class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider min-w-0 w-1/4">
                到期时间 <i class="fas fa-sort-up ml-1 text-indigo-500" title="按到期时间升序排列"></i>
              </th>
              <th scope="col" class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider min-w-0 w-1/6">
                提醒设置
              </th>
              <th scope="col" class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider min-w-0 w-1/12">
                状态
              </th>
              <th scope="col" class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider min-w-0 w-1/6">
                操作
          ...
          if (subscription.notes) {
            ...
            notesHtml = '<div class="notes-container">' +
              '<div class="notes-text text-xs text-gray-500" data-full-notes="' + notes.replace(/"/g, '&quot;') + '">' +
                truncatedNotes +
              '</div>' +
              (notes.length > 50 ? '<div class="notes-tooltip"></div>' : '') +
            '</div>';
          }

          row.innerHTML =
            '<td class="px-4 py-3">' +
              '<div class="text-sm font-medium text-gray-900 break-words">' + subscription.name + '</div>' +
              notesHtml +
            '</td>' +
            '<td class="px-4 py-3">' +
              ...
              '</div>' +
              (periodText ? '<div class="text-xs text-gray-500 mt-1">周期: ' + periodText + autoRenewIcon + '</div>' : '') +
            '</td>' +
            '<td class="px-4 py-3">' +
              '<div class="text-sm text-gray-900">' + new Date(subscription.expiryDate).toLocaleDateString() + '</div>' +
              (lunarExpiryText ? '<div class="text-xs text-blue-600 mt-1">农历: ' + lunarExpiryText + '</div>' : '') +
              '<div class="text-xs text-gray-500 mt-1">' + (daysDiff < 0 ? '已过期' + Math.abs(daysDiff) + '天' : '还剩' + daysDiff + '天') + '</div>' +
              (subscription.startDate ? '<div class="text-xs text-gray-500 mt-1">开始: ' + new Date(subscription.startDate).toLocaleDateString() + (startLunarText ? ' (' + startLunarText + ')' : '') + '</div>' : '') +
            '</td>' +
        ...
        <div class="border-b border-gray-200 pb-6">
          ...
          <div class="mb-6">
            ...
            <div class="mt-2 flex space-x-4">
              ...
              <a href="https://push.996007.icu" target="_blank" class="text-indigo-600 hover:text-indigo-800 text-sm">
          ...
          <div id="webhookConfig" class="config-section">
            ...
            <div class="grid grid-cols-1 gap-4 mb-4">
              <div>
                ...
                <input type="url" id="webhookUrl" placeholder="https://push.996007.icu/api/send/your-key" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                <p class="mt-1 text-sm text-gray-500">从 <a href="https://push.996007.icu" target="_blank" class="text-indigo-600 hover:text-indigo-800">企业微信应用通知平台</a> 获取的推送URL</p>
              </div>
              <div>
  ...
  <script>
    ...
    async function loadConfig() {
      try {
        ...
        document.getElementById('showLunarGlobal').checked = config.SHOW_LUNAR !== false;
